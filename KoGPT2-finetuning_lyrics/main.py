...
def main(epoch, save_path, load_path, samples, data_file_path, batch_size):
	...
	try:
		...
		count = int(re.findall("\d+", load_path.split('_')[1])[0]) #checkpoint이름 수정하면서 여기도 바꿈
	...
	for epoch in range(epoch):
		for data in data_loader:
			optimizer.zero_grad()
			data = torch.stack(data) # list of Tensor로 구성되어 있기 때문에 list를 stack을 통해 변환해준다. --shape문제 -> data[0]으로 수정
			...
			# generator 진행
			if (count > 0 and count % 5000 == 0) or (len(data) < batch_size): # 1000
				sent = sample_sequence(model.to("cpu"), tok, vocab, sent="사랑이", text_size=500, temperature=0.7, top_p=0.8, top_k=40)
			...
			if (count > 0 and count % 20000 == 0) or (len(data) < batch_size):  #10000
				# 모델 저장
				try:
					torch.save({
						'epoch': epoch,
						...
						'loss': loss
					}, save_path + data_file_path.split('.')[0].split('/')[1] + str(count) + '.tar')
				except:
					pass
